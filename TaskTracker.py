...
#App custom modules
import taskdb
from classes import DailyTaskWrkedHours
...
def listTask(dbConn):
    ...
    if len(activeTask) == 0: #no Active task
        ...
        for aTask in activeTask:
            ...
            logger.debug(f"aTaskID = {aTask[0]} aTaskName = '{aTask[1]}' aTrackID = {aTask[2]} aTaskDesc = '{aTask[3]}'")
            msg = f"\t{aTaskName} ({aTaskDesc})"
            print(msg)

def addingTask(dbConn,taskName="", taskDesc = ""):
    """Add a Task to the database"""
    logger.info(f"Add task '{taskName}' to database. Description: '{taskDesc}'")
...
def editTask(dbConn,orgTaskName, newTaskName=None, newTaskDesc=None):
    ...
    #Getter done
    result = taskdb.changeTask(dbConn,taskID=taskInfo[0],newName=newTaskName,newDesc=newTaskDesc)
    if result:
...
def deactivateTasks(dbConn,utc_dt, silent=False):
    ...
    if len(activeTask) == 0: #no Active task
        ...
        for aTask in activeTask:
            ...
            aTaskName = aTask[1]
            aTrackID = aTask[2]
            logger.info(f"Deactivate TaskID: {aTaskID} TaskName: '{aTaskName}' TrackID: {aTrackID}")
            taskdb.setTaskTrack(dbConn,aTaskID,utc_dt,trackID=aTrackID)
            localNow = utc_to_local(utc_dt)
            print(f"'{aTaskName}' tracking ended {localNow.strftime('%Y-%m-%d %H:%M:%S %z')}")
...
def trackTask(dbConn,taskName):
    ...
    if taskRows:
        ...
        print(f"'{taskRows[1]}' tracking started {localNow.strftime('%Y-%m-%d %H:%M:%S %z')}")
        logger.debug(f"'{taskRows[1]}' tracking local: {localNow} DBTime: {utcNow}")
    else:#Nothing found
...
def reportHours(dbConn,startDate, endDate, taskName=None,exportFile=None):
    ...
    RETURN - nothing
    """
    logger.debug(f"Getting reporting for startDate: {startDate}, endDate: {endDate}, taskName: {taskName},exportFile: {exportFile}")
    ...
    logger.debug(f"converted startDate -> startLocal: {startLocal.isoformat()}")
    ...
    if taskName: #Get official task name from database
        ...
        if taskRow: #Task found
            ...
            logger.info(f"Not able to report on task '{taskName}' - it was not found")
            print(f"Not able to find task '{taskName}'")
            return

    logger.info(f"Reporting for startUTC: {startUTC.isoformat()}, lastUTC: {lastUTC.isoformat()}, taskName: {taskName}, exportFile: {exportFile}")
    print(f"Reporting from {startLocal.strftime('%Y-%m-%d')} to {lastLocal.strftime('%Y-%m-%d')}")
    #Fetch report rows from database
    rptRows = taskdb.rptHours(dbConn,taskName=taskName,startDateUTC=startUTC,endDateUTC=lastUTC)
...
def main(parser):
    ...
    if args.command == 'list':
        ...
        reportHours(trackingDB,args.startdate,args.lastdate,taskName=args.taskName,exportFile=args.exportfile)
        ...
        if args.taskdesc:
            ...
            taskdesc = ""
        logger.info(f"Option Adding task '{args.taskname}', Desc: '{taskdesc}' ")
        addingTask(trackingDB,taskName = args.taskname, taskDesc = taskdesc)
    elif args.command == 'edit':
        logger.info(f"Option Edit task '{args.taskname}'")
        editTask(trackingDB,orgTaskName=args.taskname,newTaskName=args.newName,newTaskDesc=args.newDesc)
    sys.exit()
    # Below are going to be deprecated
    if args.new_taskName: #Adding a task
        logger.info(f"option to add a new task - task name {args.new_taskName}")
...
if __name__ == '__main__':
    ...
    addTaskGroup = addTask_parser.add_argument_group("Add Command (Adding a Task)")
    ...
    editTaskGroup = editTask_parser.add_argument_group('Edit Command (Editing a Task)')
    ...
    report_parser = commandSubparser.add_parser('report', help="Reporting working hours")
    reportTaskGroup = report_parser.add_argument_group("Report Command (Reporting working hours)")
    report_parser.add_argument('startdate', help='Start Date to report on (YYYY-MM-DD)', type=datetime.fromisoformat)
    reportTaskGroup.add_argument('-t','--task', help='Task name to report', metavar='taskname', type=str,dest='taskName')
    reportTaskGroup.add_argument('-l', '--lastdate', help='Last date for report (YYYY-MM-DD)', metavar='lastdate', type=datetime.fromisoformat, dest='lastdate')
    reportTaskGroup.add_argument('-E', '--Export', help='Export to a csv file report', metavar='exportfile', type=str, dest='exportfile')
